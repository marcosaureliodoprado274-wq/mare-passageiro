...
// Pontos fixos de Fortaleza pra teste (sem precisar de um mapa de verdade ainda)
const SPOTS = [
...
function PhoneFrame({ children }) {
  return (
    <div
      className="mx-auto rounded-[2.5rem] overflow-hidden shadow-2xl relative"
      style={{ width: 380, minHeight: 720, background: palette.ink, padding: 10, fontFamily: "Inter, sans-serif" }}
    >
...
function Pill({ children, tone = "primary", onClick, disabled, icon: Icon }) {
  const styles =
    tone === "coral"
      ? { background: palette.coral, color: palette.white }
      : tone === "ghost"
      ? { background: palette.white, color: palette.ink, border: "1px solid #D8E2DF" }
      : { background: palette.seaDeep, color: palette.white };
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="rounded-full px-5 py-3 text-sm font-semibold flex items-center justify-center gap-2 w-full transition-transform active:scale-[0.98] disabled:opacity-50"
      style={styles}
    >
      {Icon && <Icon size={16} />}
      {children}
...
// ---------------------------------------------------------------------------
// Tela de login / cadastro — troca de aba entre entrar e criar conta
// ---------------------------------------------------------------------------
function Auth({ onAuthed }) {
  ...
  async function submit() {
    ...
    try {
      let data;
      if (mode === "login") {
        data = await api.login(form.phone, form.password);
      } else {
        data = await api.register({ ...form, role: "passenger" });
      }
  ...
  return (
    <div className="flex-1 flex flex-col justify-center px-7 py-10" style={{ background: `linear-gradient(180deg, ${palette.seaDeep} 0%, ${palette.seaMid} 100%)` }}>
      ...
      <div className="bg-white rounded-2xl p-5 space-y-3">
        {mode === "register" && (
          <>
            <input placeholder="Nome completo" className="w-full border rounded-xl px-3 py-2 text-sm"
              value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} />
            <input placeholder="CPF" className="w-full border rounded-xl px-3 py-2 text-sm"
              value={form.cpf} onChange={(e) => setForm({ ...form, cpf: e.target.value })} />
            <select className="w-full border rounded-xl px-3 py-2 text-sm" value={form.city_id}
        ...
        <input placeholder="Telefone (só números)" className="w-full border rounded-xl px-3 py-2 text-sm"
          value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} />
        <input placeholder="Senha" type="password" className="w-full border rounded-xl px-3 py-2 text-sm"
          value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} />

        {error && <p className="text-xs" style={{ color: palette.safety }}>{error}</p>}

        <Pill tone="coral" onClick={submit} disabled={loading}>
          {loading ? <Loader2 size={16} className="animate-spin" /> : mode === "login" ? "Entrar" : "Criar conta"}
        </Pill>

        <button
          className="text-xs w-full text-center pt-1"
          style={{ color: palette.seaMid }}
...
// ---------------------------------------------------------------------------
// Tela: pedir corrida (com pontos fixos de Fortaleza pra teste)
// ---------------------------------------------------------------------------
  ...
  return (
    <div className="flex-1 flex flex-col p-6">
      <div className="flex items-center justify-between mb-4">
        <p className="text-lg" style={{ fontFamily: "Fraunces, serif", color: palette.ink }}>
      ...
      <select className="w-full border rounded-xl px-3 py-2 text-sm mb-3"
        value={pickup.label} onChange={(e) => setPickup(SPOTS.find((s) => s.label === e.target.value))}>
      ...
      <select className="w-full border rounded-xl px-3 py-2 text-sm mb-3"
        value={dropoff.label} onChange={(e) => setDropoff(SPOTS.find((s) => s.label === e.target.value))}>
        {SPOTS.map((s) => <option key={s.label} value={s.label}>{s.label}</option>)}
      </select>
      ...
      <div className="flex gap-2 mb-4">
        {["pix", "cartao", "especie"].map((m) => (
          <button key={m} onClick={() => setPayment(m)}
            ...
            style={{ background: payment === m ? palette.seaDeep : palette.sand, color: payment === m ? "#fff" : palette.ink }}>
            {m}
          </button>
        ))}
      </div>
      ...
      <div className="mt-auto">
        <Pill tone="coral" onClick={requestRide} disabled={loading}>
          {loading ? <Loader2 size={16} className="animate-spin" /> : "Pedir corrida"}
        </Pill>
      </div>
    </div>
  );
...
// ---------------------------------------------------------------------------
// Tela: acompanhando a corrida (consulta o status real a cada poucos segundos)
// ---------------------------------------------------------------------------
      ...
      {ride?.driver_payout && (
        <div className="mt-6 rounded-2xl p-4 w-full" style={{ background: palette.white }}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold" style={{ color: palette.seaDeep }}>Repasse ao motorista</span>
          </div>
      ...
      {ride?.risk_warnings?.length > 0 && (
        <div className="mt-3 rounded-2xl p-4 w-full flex items-start gap-2" style={{ background: "#FDEDEC" }}>
          ...
          <p className="text-xs" style={{ color: palette.ink }}>
            Esse trajeto passa por uma área com atenção redobrada nesse horário. Compartilhe seu trajeto.
          </p>
        </div>
      )}
      <button
        className="mt-6 text-xs underline"
        style={{ color: palette.safety }}
        onClick={() => api.updateRideStatus(rideId, "completed")}
      >
        (teste) simular chegada
      </button>
    </div>
  );
}
